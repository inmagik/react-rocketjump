...
The computed prop is a opt-in prop: if the configuration object or any plugin defines it, the shadow state is created and there is no way to access props that the shadow state does not expose (except when using the `mapStateToProps` parameter in the RocketJump Object connection, see [here](usage_generalities.md)). If otherwise neither the config nor any plugin define it, the original state is directly exposed. Hence, when working with plugins, pay attention whether this prop is defined or not.
...
In this example, we create a shadow state object with only the output of `getData(originalState)` exposed as a `result` property. This means we won't be able to access `isPending` or `error` without sing the `mapStateToProps` parameter in the RocketJump Object connection, see [here](usage_generalities.md).
