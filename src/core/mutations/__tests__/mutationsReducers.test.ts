import rj from '../../rj'
import { INIT, SUCCESS, FAILURE, PENDING, RUN } from '../../actions/actionTypes'
...
describe('RJ mutations reducers', () => {
  it('should be generated only when reducer key is present', async () => {
    ...
    let state = MaRjState.reducer(undefined, { type: INIT })
  ...
  it('should be optimistic and honored the mutations state', () => {
    function singleMutationReducer(
      ...
      switch (action.type) {
        case PENDING: {
          const rcount = state.rcount + (action?.meta?.mutationID ?? Infinity)
          return {
            ...state,
            rcount,
            error: null,
