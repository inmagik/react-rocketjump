import  bindActionCreators from '../../../core/actions/bindActionCreators'
import { rj, makeEffectAction } from '../../..'
import rjDebounce from '..'
import { createTestRJSubscription } from '../../../core/testUtils'
import { PENDING, SUCCESS, CLEAN, RUN } from '../../../core/actions/actionTypes'
...
describe('rjDebounce', () => {
  ...
  it('should ingore debounce on non debounced action', (done) => {
    ...
    const subject = createTestRJSubscription(RjObject, mockCallback)
    const dispatch = (action: any) => subject.next(action)
    const { run, drago } = bindActionCreators(actionCreators, dispatch)
    drago()
    drago()
    run()
    run()
