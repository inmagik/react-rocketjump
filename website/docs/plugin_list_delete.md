...
- **identity**: this function is called once for every element in the list, and is expected to return `true` if the item passed in along with the deleting action is the target of the delete operation (`(action, listItem) => bool` ). Default value is `(action, listItem) => action.item.id === listItem.id`
...
- **deleteItem** (`deleteItem(item)`): deletes zero or more items from the list, according to the `identity` function
